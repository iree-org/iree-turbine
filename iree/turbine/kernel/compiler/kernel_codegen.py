...
class BindingDesc:
    ...
    def as_mlir_type(self) -> IrType:
        ...
        if binding_type == BindingType.KERNEL_BUFFER:
            ...
            # If a physical layout is specified, that means the logical and physical
            # shapes/strides are not the same. So we use the physical layout to determine
            # the memref type.
            ref_type = self.reference[1].type
            if ref_type.physical_layout:
                strides = _get_mixed_stride(ref_type.physical_layout.strides)
                shape_asm = "x".join(
