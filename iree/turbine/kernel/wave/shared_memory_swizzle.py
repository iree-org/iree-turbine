...
from ..ops.wave_ops import get_custom, Read, Write, wave_xor
from dataclasses import dataclass
from .._support.indexing import IndexExpr, IndexSequence
...
def swizzle_index(
    ...
    vec_size = shmem_write.elements_per_thread
    max_phase = max(
        min(
    ...
    swizzled_col = wave_xor(col, (row // per_phase) % max_phase) * vec_size + (
        col % vec_size
    )

    shmem_write.index[inner_dim.dim].start = swizzled_col
    for read in shmem_reads:
        if read in already_swizzled:
            continue
        read.index[inner_dim.dim].start = swizzled_col
