...
def emit_global_scan(
    ...
    if local_scan_size > 1:
        ...
        for idx, loc_scan in enumerate(local_scan):
            ...
            scanop_result.index = get_custom(loc_scan[0]).index
            scanop_result.expanded_dims = get_custom(loc_scan[0]).expanded_dims
            scanop_result.vector_shapes = get_custom(loc_scan[0]).vector_shapes

            final_scanop_result.append(scanop_result)

    return final_scanop_result if final_scanop_result else scanop_result
...
def decompose_scan_ops(
    ...
    for node in scan_nodes:
        ...
        with custom.graph.inserting_before(custom.fx_node):
            ...
            if not isinstance(global_scan, list):
                global_scan = [global_scan]

            # check scan dim and update the user
            # for usr_idx in range(0, len(custom.users)):
            #     breakpoint()
            #     custom.users[usr_idx].update_arg(custom.users[usr_idx].fx_node.args.index(custom.fx_node),
            #         global_scan[custom.users[usr_idx].expanded_dims[scan_dim]])
            ...
            for user, arg_index in users:
                ...
                # user.update_arg(arg_index, global_scan[0])
