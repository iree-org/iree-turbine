...
\usepackage{tikz}
\usetikzlibrary{arrows, positioning, fit, shapes, backgrounds}
\usepackage{dot2texi}
...
\begin{tikzpicture}[>=latex',scale=0.45]
    \tikzstyle{n} = [draw,shape=ellipse,inner sep=1pt,fill=black!20]
    \tikzstyle{every node}=[font=\footnotesize]
    \input{dot_files/gemm_traced_initial_converted}
\end{tikzpicture}%
\hspace{0.3cm}
\begin{tikzpicture}[>=latex',scale=0.45]
    % set node style
    \tikzstyle{n} = [draw,shape=ellipse,inner sep=1pt,fill=black!20]
    \tikzstyle{every node}=[font=\footnotesize]
    % Generate this file (and the one above) by copying the dot file and running (on macos):
    % pbpaste | dot2tex -f tikz --codeonly | pbcopy
    % The result will be in your clipboard
    \input{dot_files/gemm_expanded_converted}
    % Note: Conversion can also be done by the package directly but then its harder to see the errors
    % \begin{dot2tex}[dot,tikz,codeonly,styleonly,options=-s]
    %     \input{dot_files/gemm_expanded.dot}
    % \end{dot2tex}
    \tikzstyle{highlight} = [rounded corners=0.5em,line width=1em,cap=round]

    % Annotations in the background
    \begin{pgfonlayer}{background}
        % Expansion 1: dims: (0,1,0)?
        \begin {scope}[draw=red!20,ultra thick]
            \draw[highlight] (b.center) -- (mma010.center);
            \draw[highlight] (b.center) -- (read010.center) -- (mma010.center);
            \draw[highlight] (acc010.center) -- (mma010.center);
        \end{scope}

        % Expansion 2: dims: (1,0,0)?
        \begin {scope}[draw=blue!20,ultra thick]
            \draw[highlight] (a.center) -- (read100.center) -- (mma100.center);
            \draw[highlight] (acc100.center) -- (mma100.center);
        \end{scope}

        % Expansion 3: dims: (1,1,0)?
        \begin {scope}[draw=green!20,ultra thick]
            \draw[highlight] (read100.center) -- (mma110.center);
            \draw[highlight] (read010.center) -- (mma110.center);
            \draw[highlight] (acc110.center) -- (mma110.center);
        \end{scope}

        % TODO: Generate the intermediate graphs of the expansion of the reduction dimension and add the colorings to the figure as well.
        % TODO: double check the expand dims I have here

        % TODO: We are not showing the complete graph here, only the subgraph of the reduction, which might be confusing. Simply addind the different writes instead of the output node might improve this already.
    \end{pgfonlayer}
\end{tikzpicture}
...
\end{figure*}
% TODO: Make this a proper subfigure with a) and b)
