...
def get_node_type(node: fx.Node) -> str:
    ...
    if isinstance(custom, (Read, Write)):
        ...
        memory_type = (
            "Global"
            if hasattr(custom, "address_space")
            and custom.address_space == GLOBAL_ADDRESS_SPACE
            else "Shared"
        )
...
def _calculate_column_widths(
    schedule_data: list[Tuple[fx.Node, int, int, str, list[str]]]
) -> dict[str, int]:
    ...
    for node, cycle, stage, node_type, user_sort_keys in schedule_data:
...
def dump_schedule(
    ...
    """
    Dumps the schedule to a file in a pipe-delimited table format.
    Each row contains: node_name | node_type | node_sort_key | cycle | stage | user_sort_keys
    ...
    for node, cycle in schedule.items():
        ...
        schedule_data.append((node, cycle, stage, node_type, user_sort_keys))
...
def load_schedule(
    ...
    for line in data_lines:
        # Parse all columns
        name, _, sort_key_str, cycle_str, _, user_sort_keys_str = [
