...
def get_decode_attention_kernels(
    ...
    def phase_0(
        ...
        reciprocal_sum = tkw.reciprocal(d_j)
        res = acc * reciprocal_sum
...
# In these kernels, we introduce the artificial constraint that when loading
# the key and value tensors, we can only load them in chunks of a fixed size.
# This is to simulate what happens in paged attention where the
# kv-cache stores the keys and values in chunks of a fixed size.
def get_blocked_decode_attention_kernels(
    shape: tuple[int],
    mfma_variant: MMAType,
    use_dynamic_dims: bool = False,
):
    ...
    def phase_0_constraints():
        ...
        constraints += [tkw.WorkgroupConstraint(K2, BLOCK_K2, 2)]
        constraints += [tkw.WaveConstraint(K2, BLOCK_K2 / K_WAVES)]
        constraints += [tkw.TilingConstraint(K2, BLOCK_K2 / K_WAVES / NUM_CHUNKS)]
        constraints += [tkw.WorkgroupConstraint(B, BLOCK_B, 3)]
        constraints += [
            SymbolicAlias(U, K2, lambda x: sympy.ceiling(x / (BLOCK_K2 / K_WAVES)))
        ]
    ...
    # The kernel below will not work for key/value in shared memory because
    # of the discussion in constraints.py/get_constrained_shape.
    @tkw.wave(get_constraints(Phase.PHASE_0))
    def phase_0(
        q: tkl.Memory[B, M, K1, GLOBAL_ADDRESS_SPACE, tkl.f16],
