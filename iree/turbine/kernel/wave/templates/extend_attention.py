...
def get_extend_attention_kernel(
    ...
    has_custom_mask: Optional[bool] = False,
):
    ...
    custom_mask_mapping = tkw.IndexMapping(
        num_iterators=1,
        inputs={N_KV: i + MASK_START_IDX + SEQ_LEN * N_Q_IDX},
        outputs={N_KV: i},
    )
    ...
    def extend_attention_custom_mask(
        ...
        kv_indptr: tkl.Memory[S, GLOBAL_ADDRESS_SPACE, tkl.i32, num_seqs_layout],
        kv_indices: tkl.Memory[N_KV, GLOBAL_ADDRESS_SPACE, tkl.i32, kv_indices_layout],
        custom_mask: tkl.Memory[N_KV, GLOBAL_ADDRESS_SPACE, tkl.bool, num_seqs_layout],
        ...
        def first_loop(
            ...
            n_kv_index = tkw.self_index(N_KV, tkl.i32)
            n_q_index = tkw.self_index(N_Q, tkl.i32)
            tkw.set_symbol(N_Q_IDX, n_q_index)
            c_mask = tkw.read(custom_mask, elements_per_thread=STORE_ELEMS_PER_THREAD, mapping=custom_mask_mapping)
            mask = tkw.apply_expr(n_kv_index, lambda x: x < N_KV)
            mask = tkw.broadcast(mask, target_shape=[N_Q, N_KV])
            mask = tkw.cast(mask, tkw.i1)
            mask &= c_mask
    ...
    if has_custom_mask:
        return extend_attention_custom_mask, hyperparams, dynamic_symbols, dynamic_symbols_map
