...
def _valid_bytes_buffer(elem_type):
    """
...
def _get_out_of_bounds_index(element_type):
    """
...
def _cast_buffer_and_encode_stride(ptr, strides, elem_type):
    ...
    CACHE_SWIZZLE_ENABLED = False
    if CACHE_SWIZZLE_ENABLED:
...
def _create_vec_read_write(
    ...
    no_masked_load_store_ops = buffer_ops_enabled and has_int_strides
    ...
    if mask is None and offsets_vec is None:
        ...
        if buffer_ops_enabled:
            ...
            mem = _cast_buffer_and_encode_stride(mem, strides, element_type)
