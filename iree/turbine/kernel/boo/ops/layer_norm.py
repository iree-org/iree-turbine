...
def test():
    input = torch.randn([4, 32, 42, 32], dtype=torch.bfloat16)
    reference = torch.layer_norm(input, input.shape[1:])
    boo = boo_layer_norm(input, input.shape[1:])
    if not torch.allclose(boo, reference):
        print("Mismatch.")
