...
def get_module_asm(
    ...
    mlir_path = cache_files.mlir_path
    ...
    if is_cache_enabled():
        ...
        cache_files.base_dir.mkdir(parents=True, exist_ok=True)
...
def out_of_process_compile(
    ...
    for key_hash, flags in key_hashes_and_flags:
        vmfb_path = cache_files.vmfb_path(key_hash)
        ...
        cache_files.compile_command_log(key_hash).write_text(command)
...
def user_flags_jit_callback(
    ...
    def _compile(
        ...
        if ret.returncode != 0:
            raise RuntimeError(
                f"Failed compilation for kernel {func_name}. "
    ...
    def callback(device):
        ...
        mlir_path = cache_files.mlir_path
        vmfb_path = cache_files.vmfb_path(key_hash)
        command_log_path = cache_files.compile_command_log(key_hash)
    ...
    *,
    ...
    if cache_only:
        ...
        launch = Launchable.from_file_cache_only(
            OpCacheFiles(func_name).base_dir,
            parameter_providers=(),
...
def get_launchable_from_graph_module(
    ...
    *,
    ...
    LaunchableRuntimeCache.add(func_name, launch)
    return launch
