...
class LayerNormSignature(OpSignature):
    ...
    def as_init_kwargs(self) -> dict[str, Any]:
        return self._signature._asdict()
...
class LayerNormBackwardInput(torch.nn.Module):
    ...
    # def forward(self, dLdy, x: torch.Tensor, weight: torch.Tensor):
    #     # TODO: this is a manual implementation of the gradient since it wasn't
    #     # clear to me how to convince autograd to give me a graph compatible
    #     # with exports. The first block computes values that we would normally
    #     # store in the forward pass of a manual layernorm implementation.

    #     mean = x.sum(dim=self.normalized_dim, keepdim=True) / math.prod(self.normalized_shape)
    #     offset = x - mean
    #     variance = (offset**2).sum(dim=self.normalized_dim, keepdim=True)
    #     stddev = torch.sqrt(variance + self.eps)
    #     normalized = offset / stddev

    #     input
    #     mean
    #     rstddev

    #     dNormalized = dLdy * weight # TODO: check if weight is always provided
    #     dx = dNormalized - dNormalized.mean(dim=self.normalized_dim, keepdim=True) - normalized * (dNormalized * normalized).mean(dim=self.normalized_dim, keepdim=True)
    #     dx /= stddev
    #     return dx
...
class LayerNormBackwardWeight(torch.nn.Module):
    ...
    #     dWeight = (dLdy * normalized).sum(dim=self.keep_dim)
    #     return dWeight
