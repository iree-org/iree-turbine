...
default_test_shapes["test_paged_decoding"]
default_test_shapes["test_paged_decoding"] = [(32, 6, 0, 128, 1025)]
...
def testPagedFlashDecoding(
    ...
    def qk_kernel(
        ...
        request_to_tokens: tkl.Memory[
            B, GLOBAL_ADDRESS_SPACE, tkl.i32, req_to_tokens_layout
        ],
    ...
    q = device_randn(1, shape[0] * shape[1], shape[3], dtype=torch.float16)
    q = torch.as_strided(q, q.shape, (SEQ_LEN, HEAD_DIM, 1))
    ...
    request_indices = torch.arange(BATCH, dtype=torch.int32).to_default_device()
    request_offsets = torch.arange(BATCH).to_default_device()
    k_cache = device_randn(total_entries, 1, 1, shape[3], dtype=torch.float16)
