...
def test_batched_gemm():
    ...
    with tk.gen.TestLaunchContext(
        ...
        # CHECK:          func.func @batched_gemm(%[[ARG0:.*]]: !stream.binding, %[[ARG1:.*]]: !stream.binding,
        # CHECK-SAME:       %[[ARG2:.*]]: !stream.binding) attributes {translation_info =
        ...
        # CHECK:            %[[D15:.+]] = arith.addi %[[D14]], %[[D12]] : index
        # CHECK:            %[[D16:.+]] = arith.addi %[[D5]], %[[D12]] : index
        # CHECK:            %[[D17:.+]] = scf.for %[[ARG3:.*]] = %[[C0]] to %[[C4]] step %[[C1]]
        # CHECK-SAME:         iter_args(%[[ARG4:.*]] = %[[CST]]) -> (vector<4xf32>) {
        # CHECK:              %[[D39:.+]] = arith.muli %[[ARG3]], %[[C16]] : index
        # CHECK:              %[[D40:.+]] = arith.addi %[[D39]], %[[D10]] : index
