...
def test_flash_paged_decoding():
    ...
    k_mapping = tkw.IndexMapping(
        ...
        inputs={
            B: d0,
            M: j / sympy.ceiling(NUM_HEADS / HEAD_TILE_SIZE),
            K2: sympy.Integer(0),
            K1: l,
        },
        outputs={B: i, M: j, K2: k, K1: l},
        dynamic_val_mappings={B: i // LOAD_ELEMS_PER_THREAD, K2: k},
    )
    ...
    k_layout = {"stride": (K1, K1, K1, 1), "shape": (BK, 1, 1, K1)}
    req_to_tokens_layout = {"stride": (R1, 1), "shape": (R0, R1)}

    @tkw.wave(get_constraints(Phase.QK))
    def qk_kernel(
        q: tkl.Memory[B, M, K1, ADDRESS_SPACE, tkl.f16, q_layout],
        k: tkl.Memory[B, M, K2, K1, ADDRESS_SPACE, tkl.f16, k_layout],
