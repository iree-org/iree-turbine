...
@run_test
def test_flash_paged_decoding():
    shape = (32, 6, 128, 128, 1025)
    ...
    def get_constraints(phase: Phase) -> list[tkw.Constraint]:
        ...
        n_ratio = 2
        if phase == Phase.QK:
            # Distribute the batch, head and sequence length dimensions across the workgroups.
            constraints += [tkw.WorkgroupConstraint(K2, BLOCK_K2, 1)]
            constraints += [tkw.WaveConstraint(K2, BLOCK_K2 / n_ratio)]
            vector_shapes = {B: 0, M: 16, K2: 16}
        else:
            constraints += [tkw.WorkgroupConstraint(N, BLOCK_N, 1)]
            constraints += [tkw.TilingConstraint(K2, BLOCK_K2)]
            constraints += [tkw.WaveConstraint(N, BLOCK_N / n_ratio)]
            vector_shapes = {B: 0, M: 16, N: 16}
        constraints += [
            tkw.HardwareConstraint(
                threads_per_wave=64,
                waves_per_block=(m_ratio, n_ratio, 1),
                mma_type=mfma_variant,
                vector_shapes=vector_shapes,
            )
        ]
    ...
    # The k-v cache has logical dimensions [B, M, K1] and we
    # broadcast it to [B, K2, K1].
    # TODO: Explicitly adding WORKGROUP_0 here only works if we
    # are not distribution to waves. In general, we will want to replace
    # this with the index corresponding to dimension M.
    k_mapping = tkw.IndexMapping(
        num_iterators=3,
        inputs={
        ...
        # constants={M: l}, and then we add M to the inputs.
    ...
    # The first kernel computes Q @ K.T, after loading K from the K-cache.
    # Say the batch dimension B = 32, num heads M = 6 and head dimension K1 = 128.
    # The shape of the Q matrix is [32, 6, 128] / [16, 128] (BLOCK_B: 1, BLOCK_M: 16).
    # The K cache has a much larger first dimension that could be on the order of
    # ~O(10^7). Since we wil always be accessing this dimension using a dynamic variable,
    # and will always be loading [K2, K1] from the K cache, we can set the first dimension
    # to be B.  # After loading the K matrix, it would be of shape : [32, ?, 128] / [64, 128]
    # (with a BLOCK_K2 = 64) where we are using the same batch dimension for all the K vectors.

    # The request to tokens matrix maps from logical to physical indices in the K cache.
    # In general it has a shape [R0, R1], but since we are only loadin B offsets we represent it
    # with shape [B] and use R0, R1 in the mapping to access the appropriate index. In general,
    # we can always represent an n-D matrix as a 1-D matrix.
    # The request indices and offsets have shape [B] and are used to index into the request_to_tokens matrix.

    # Finally, the output matrix is of shape [B, M, K2] where K2 is the maximum sequence length.
    # (So it would be of shape [32, 6, 128] / [16, 64]).

    # Define the physical storage layout of the input memory buffers. They do not have to
    # be identical to the logical storage layout specified by the symbolic dimensions.
    ...
    # @tkw.wave(get_constraints(Phase.SOFTMAX_V))
    # def softmax_v_kernel(
    #    qk: tkl.Memory[B, H, M, K2, ADDRESS_SPACE, tkl.f32],
    #    v: tkl.Memory[BV, K2, ADDRESS_SPACE, tkl.f16],
    #    request_to_tokens: tkl.Memory[R0, R1, ADDRESS_SPACE, tkl.f16],
    #    request_indices: tkl.Memory[B, ADDRESS_SPACE, tkl.i32],
    #    request_offsets: tkl.Memory[B, ADDRESS_SPACE, tkl.i32],
    #    output: tkl.Memory[B, H, M, N, GLOBAL_ADDRESS_SPACE, tkl.f32],
    # ):
    ...
    #    ) -> (
    #        tkl.Register[B, H, M, tkl.f32],
    #        tkl.Register[B, H, M, tkl.f32],
    #        tkl.Register[B, H, N, M, tkl.f32],
    #    ):
    #        x_j = tkw.read(qk, elements_per_thread=STORE_ELEMS_PER_THREAD)
    #        m_j = tkw.max(x_j, partial_max, dim=K2)
    #        e_delta_max = tkw.exp2(partial_max - m_j)
    ...
    #    res = res_mm / res_sum
    #    tkw.write(
    #        res, output, mapping=mapping, elements_per_thread=STORE_ELEMS_PER_THREAD
    #    )
    ...
    hyperparams = {
        ...
        STORE_ELEMS_PER_THREAD: get_mfma_store_elems_per_thread(mfma_variant),
        BLOCK_B: 1,
        BLOCK_M: 16,  # Read heads in blocks of 16
        BLOCK_N: 64,
        BLOCK_K2: 64,  # Sequence length in blocks of 64
        B: shape[0],
        M: shape[1],
        N: shape[2],
        K1: shape[3],
